...
.controller('ArquivoController', ['$scope', '$localStorage', 'ArquivoService', '$timeout', '$filter', 'Utils',
	function ($scope, $localStorage, ArquivoService, $timeout, $filter, Utils) {
    ...
    $scope.getAllByPacienteID = function(){

        ArquivoService.getAllByPacienteID($scope.objeto.pacienteID)
        .success(function(response){
            if(response=="erro"){
                console.info("erro");
            }else{
                if(response.arquivo.length>0){
                    $scope.objetos = response.arquivo;
                    tratarResposta();
                }else{
                    $scope.objetos = new Object();
                }
    ...
    var tratarResposta = function(){
        for(i=0; i<$scope.objetos.length; i++){
            ...
            $scope.objetos[i].url = tratarNomeArquivo($scope.objetos[i].url);
            $scope.objetos[i].url = $scope.objetos[i].url.replaceAll('%C3%A2', 'a%CC%82')
        }
    }
    ...
    var tratarNomeArquivo = function(url){
    ...
    $scope.salvar = function(){
        $scope.objeto.pacienteID = $scope.local.pacienteID;
        $scope.objeto.updated_at = Utils.generateDateToDataBase();

        if($scope.objeto.id){
            ArquivoService.update($scope.objeto)
            .success(function(response){
                if(response){
            ...
            ArquivoService.create($scope.objeto)
            .success(function(response){
    ...
    //     ArquivoService.getByPacienteID(pacienteID)
    //     .success(function(response){
    ...
    var excluir = function(){
        ArquivoService.delete($scope.objeto.id)
        .success(function(response){
            if(response==1){
    ...
    $scope.download = function(file){
        ArquivoService.download(GLOBAL.barramentoDownload + "/" + file.url.trim(), file.url.trim());
    }
    ...
    init = function() {
        if($scope.$parent.$parent && $scope.$parent.$parent.objeto && $scope.$parent.$parent.objeto.id){
            ...
            $scope.local.pacienteID = pacienteID;
            $scope.getAllByPacienteID();
        }
    };
